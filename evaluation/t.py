...
train_data = [
    [mx.array(arr[idx : idx + batch_size]) for arr in all_data]
    for idx in range(0, n_train_data - batch_size, batch_size)
]
train_labels = [
    mx.array(all_labels[idx : idx + batch_size])
    for idx in range(0, n_train_data - batch_size, batch_size)
]
print(f"n_train_data={n_train_data}")
...
test_labels = [
    mx.array(all_labels[idx : idx + test_batch_size])
    for idx in range(n_train_data, len_data - test_batch_size, test_batch_size)
]
...
print(f"len test_data[0]={len(test_data[0])}")
for idx in range(len(test_data)):
    print(f"test_data[{idx}][0].shape={test_data[idx][0].shape}")
print(f"len test_data={len(test_labels)}")
print(f"len test_data[0]={len(test_labels[0])}")
for idx in range(len(test_labels)):
    print(f"test_data[{idx}].shape={test_labels[idx].shape}")
